...
void backpropagation()
{
  ...
  for (int i = 0; i<n_outputs;i++){
    gradients_labels[backup.correct_label].output_layer_bias_grad[i] = o_error[i]*learning_rate;
    for (int j = 0; j<n_hidden_2;j++){
      gradients_labels[backup.correct_label].output_weights_grad[j][i] =  learning_rate*o_error[i]*backup.hidden_layer_2_ex[j];
    }
  }
  //Adjusts the Hidden_1-Hidden_2 Weights and Bias
  for (int i = 0; i<n_hidden_2;i++){
    gradients_labels[backup.correct_label].hidden_layer_2_bias_grad[i] = H2_error[i]*learning_rate;
    for (int j = 0; j<n_hidden_1;j++){
      gradients_labels[backup.correct_label].hidden_weights_2_grad[j][i] = learning_rate*H2_error[i]*backup.hidden_layer_1_ex[j];
    }
  }
  //Adjusts the Input-Hidden_1 Weights and Bias
  for (int i = 0; i<n_hidden_1;i++){
    gradients_labels[backup.correct_label].hidden_layer_1_bias_grad[i] = H1_error[i]*learning_rate;
    for (int j = 0; j<n_inputs;j++){
      gradients_labels[backup.correct_label].hidden_weights_1_grad[j][i] = learning_rate*H1_error[i]*backup.input_layer_ex[j];
    }
  }
}

void backer_Up(double desired[], int label){
  //Backs Up Current Forward Pass Values

  for(int i = 0; i<n_outputs;i++){
    perc_distr[backup.correct_label][i] = backup.output_layer_ex[i];
  } //Lets Try Moving this here after it updates
  ...
  //printf("PreBackUp %d: \n",backup.correct_label);
  backup.correct_label = label; 
  //printf("AfterBackUp %d: \n",backup.correct_label);
}
...
void weight_updates(){
  ...
  for (int i = 0; i<n_outputs;i++){
    N_Network.output_layer_bias[i] += gradients_labels[backup.correct_label].output_layer_bias_grad[i];
    for (int j = 0; j<n_hidden_2;j++){
      N_Network.output_weights[j][i] += gradients_labels[backup.correct_label].output_weights_grad[j][i];  
    }
  }
  //Applies Hidden Layer 2 Gradients
  for (int i = 0; i<n_hidden_2;i++){
    N_Network.hidden_layer_2_bias[i] += gradients_labels[backup.correct_label].hidden_layer_2_bias_grad[i];
    for (int j = 0; j<n_hidden_1;j++){
      N_Network.hidden_weights_2[j][i] += gradients_labels[backup.correct_label].hidden_weights_2_grad[j][i];
    }
  }
  //Applies Hidden Layer 1 Gradients
  for (int i = 0; i<n_hidden_1;i++){
    N_Network.hidden_layer_1_bias[i] += gradients_labels[backup.correct_label].hidden_layer_1_bias_grad[i];
    for (int j = 0; j<n_inputs;j++){
      N_Network.hidden_weights_1[j][i] += gradients_labels[backup.correct_label].hidden_weights_1_grad[j][i];
    }
  }
}
    ...
    for(int i = 0; i < im_Num; i++){  
      ...
      #pragma omp parallel num_threads(2)
      {
        ...
        #pragma omp single
        {
          ...
          if(speculative_Backpropagation == 'Y' && cross_entrpy(perc_distr[backup.correct_label],backup.output_layer_ex) >= threshold)  //If The Difference between the Previous Outputs and Perc Distr @ Label is small enough
          {
            backpropagation();
          }
        }
